...
def history_path(symbol):
    ...
    return os.path.join(HISTORY_DIR, f"{symbol}_full_{date_str}.csv")
...
st.title(f"{TARGET_SYMBOL} - Full Dashboard (prototype)")
...
# --- Daily data + indicators (cash market - needs long history) ---
candles = fetch_daily_candles(cash_key, access_token)
closes = [c[4] for c in candles]
dates = [c[0][:10] for c in candles]  # YYYY-MM-DD from the timestamp
...
st.subheader("Daily Chart - EMA, Bollinger Bands, S/R Zones")
fig = go.Figure()
